...
// Route to verify a salon license
router.post('/verify/:salonId', async (req: Request, res: Response) => {
  try {
    ...
    // Update the salon's license status to verified
    const updatedSalon = await storage.updateSalonLicense(Number(salonId), {
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        licenseName: updatedSalon.licenseName,
        licenseNumber: updatedSalon.licenseNumber,
        licenseState: updatedSalon.licenseState,
        licenseStatus: updatedSalon.licenseStatus,
        licenseVerified: updatedSalon.licenseVerified
      }
    });
  } catch (error) {
...
// Route to reject a salon license
router.post('/reject/:salonId', async (req: Request, res: Response) => {
  try {
    ...
    // Update the salon's license status to rejected
    const updatedSalon = await storage.updateSalonLicense(Number(salonId), {
      licenseStatus: 'rejected',
      licenseVerified: false,
      licenseRejectionReason: reason || 'License information could not be verified.',
      licenseVerificationDate: new Date().toISOString()
    });
    ...
    return res.status(200).json({
      success: true,
      message: 'License successfully rejected.',
      data: {
        salonId: updatedSalon.id,
        licenseName: updatedSalon.licenseName,
        licenseNumber: updatedSalon.licenseNumber,
        ...
        licenseStatus: updatedSalon.licenseStatus,
        licenseVerified: updatedSalon.licenseVerified,
        licenseRejectionReason: updatedSalon.licenseRejectionReason
      }
    });
  } catch (error) {
