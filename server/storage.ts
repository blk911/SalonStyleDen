...
export class DatabaseStorage implements IStorage {
  ...
  // Get invitations received by a client
  async getClientReceivedInvitations(clientId: number, status?: string, limit?: number): Promise<Invitation[]> {
    try {
      console.log(`DatabaseStorage.getClientReceivedInvitations - Fetching received invitations for client ${clientId}${status ? ` with status ${status}` : ''}`);
      
      // First get the client's phone number
      const clientData = await this.getClient(clientId);
      if (!clientData) {
        console.error(`DatabaseStorage.getClientReceivedInvitations - Client ${clientId} not found`);
        return [];
      }
      ...
      // Get invitations where the client is the recipient (by phone)
      queryParams.push(`phone LIKE $${values.length + 1} AND sender_id != $${values.length + 2}`);
      values.push(`%${clientPhone}%`); // Use LIKE with wildcards for flexible matching
      values.push(clientId); // Ensure the client is not the sender
      ...
      try {
        const result = await client.query(sqlQuery, values);
        const rows = result.rows;
        console.log(`DatabaseStorage.getClientReceivedInvitations - Retrieved ${rows.length} received invitations for client ${clientId}`);
      ...
      console.error(`DatabaseStorage.getClientReceivedInvitations - Error fetching received invitations for client ${clientId}:`, error);
      throw error;
    }
  }
  
  // Get invitations sent by a client
  async getClientSentInvitations(clientId: number, status?: string, limit?: number): Promise<Invitation[]> {
    try {
      console.log(`DatabaseStorage.getClientSentInvitations - Fetching sent invitations for client ${clientId}${status ? ` with status ${status}` : ''}`);
      
      // Build the query parameters list and values array
      const queryParams: string[] = [];
      const values: any[] = [];
      
      // Get invitations where the client is explicitly the sender
      ...
      try {
        const result = await client.query(sqlQuery, values);
        const rows = result.rows;
        console.log(`DatabaseStorage.getClientSentInvitations - Retrieved ${rows.length} sent invitations for client ${clientId}`);
      ...
      console.error(`DatabaseStorage.getClientSentInvitations - Error fetching sent invitations for client ${clientId}:`, error);
      throw error;
    }
  }
