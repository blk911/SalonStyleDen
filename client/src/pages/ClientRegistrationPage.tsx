    ...
    <div className="flex flex-col min-h-screen">
      ...
      <Dialog 
        ...
        onOpenChange={(open) => {
          ...
          // If we're closing the dialog and it's not a gift case
          if (!open && !hasUnredeemedGift) {
            // Mark dialog as shown to prevent reopening
            setAddressDialogShown(true);
            document.body.setAttribute('data-address-shown', 'true');
            
            // Give dialog time to close properly
            setTimeout(() => {
              // Focus terms checkbox after dialog closes
              focusTermsCheckbox();
            }, 50);
          }
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <select
                          ...
                          value={field.value || ''}
                          onChange={(e) => {
                            // Stop propagation to prevent dialog interference
                            e.stopPropagation();
