...
export default function SalonDashboard() {
  ...
  const licenseFormSchema = z.object({
    ...
    licenseNumber: z.string().min(2, {message: "Please enter your license number"}),
    licenseState: z.string().min(2, {message: "Please select a state"}),
    exactMatch: z.boolean().refine(val => val === true, {
      message: "You must verify this information is accurate"
    }),
    agreeToTerms: z.boolean().refine(val => val === true, {
      message: "You must agree to the license verification terms"
    })
  });
  ...
  const handleLicenseSubmission = async (data: LicenseFormValues) => {
    try {
      ...
      await apiRequest(`/api/salons/${id}/license`, {
        method: 'POST',
        data: {
      ...
      toast({
        ...
        description: "Your license information has been submitted for verification. This typically takes 2-3 business days.",
        duration: 5000
      });
      ...
      toast({
        title: "Error",
        description: "Failed to submit your license information. Please try again.",
        variant: "destructive",
        duration: 3000
      });
    }
